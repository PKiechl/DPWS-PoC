 ...
 * This implementation by the above author (Saulo da Mata) was modified to allow for the reading time limit to be set
 * to a specific value, given that the default may not be practical for shorter simulations.
 */
    ...
    static TypeId tid =
        TypeId("ns3::HttpClient")
            ...
            .AddAttribute("MaxReadingTime",
                          "Override the preconfigured reading time limit (max. time between fully "
                          "receiving requested object and requesting the next one)",
                          UintegerValue(10000),
                          MakeUintegerAccessor(&HttpClient::m_maxReadingTime),
        ...
        if (m_bytesReceived == m_contentLength)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    // Traffic Model Based on the Top One Million Visited Web Pages" by Rastin Pries
                    // et. al (Table II).
                    // Modification: Parametrized max reading time
                    if (readingTime > m_maxReadingTime)
                    {
                        readingTime = m_maxReadingTime;
                    }

                    NS_LOG_DEBUG("HttpClient (" << m_clientAddress << ") >> Reading time: "
                                                << readingTime << " seconds.");
                    Simulator::Schedule(Seconds(readingTime),
